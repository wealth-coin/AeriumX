...
#if defined(NDEBUG)
#error "WealthSilo cannot be compiled without assertions."
#endif
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zWEALTH with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zWEALTH spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zWEALTH spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
//9 JUN zee comit
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 272800 && nHeight > 10000) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 535600 && nHeight > 272800) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 798400 && nHeight > 535600) {
        nSubsidy = 5 * COIN;
    } else if (nHeight <= 1061200 && nHeight > 798400) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight <= 132400 && nHeight > 1061200) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight <= 1586800 && nHeight > 132400) {
        nSubsidy = 0.625 * COIN;
    } else if (nHeight <= 1849600 && nHeight > 1586800) {
        nSubsidy = 0.3125 * COIN;  
    } else if (nHeight <= 2112400 && nHeight > 1849600) {
        nSubsidy = 0.3125 * COIN;       
    } else if (nHeight > 2112400) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 200) {
        ret = 0;
    } else if (nHeight >= 2112400 && nHeight <= 200) {
        ret = blockValue * .75;
    } else {
        ret = blockValue * .75;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from WealthSilo
...
{
    RenameThread("wealthsilo-scriptch");
...
void RecalculateZWEALTHSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zWEALTH supply
...
bool RecalculateWEALTHSupply(int nHeightStart)
{
    ...
    if (nHeightStart == Params().Zerocoin_StartHeight())
        nSupplyPrev = CAmount(3050000000000000);
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert WEALTH to zWEALTH
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zWEALTH spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zWEALTH serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zWEALTH mints and %d zWEALTH spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WealthSilo: We use certain sporks during IBD, so check to see if they are
