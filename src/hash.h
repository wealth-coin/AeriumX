...
GLOBAL sph_groestl256_context   z_groestl;
GLOBAL sph_jh256_context        z_jh;
GLOBAL sph_keccak256_context    z_keccak;
GLOBAL sph_skein256_context     z_skein;
GLOBAL sph_luffa256_context     z_luffa;
GLOBAL sph_cubehash256_context  z_cubehash;
GLOBAL sph_shavite256_context   z_shavite;
GLOBAL sph_simd256_context      z_simd;
GLOBAL sph_echo256_context      z_echo;
...
#define fillz() do { \
    ...
    sph_groestl256_init(&z_groestl); \
    sph_jh256_init(&z_jh); \
    sph_keccak256_init(&z_keccak); \
    sph_skein256_init(&z_skein); \
    sph_luffa256_init(&z_luffa); \
    sph_cubehash256_init(&z_cubehash); \
    sph_shavite256_init(&z_shavite); \
    sph_simd256_init(&z_simd); \
    sph_echo256_init(&z_echo); \
...
template<typename T1>
inline uint256 Aergo(const T1 pbegin, const T1 pend)
{
	...
	sph_simd512(&ctx_simd, static_cast<const void*>(&hash), 64);
	sph_simd512_close(&ctx_simd, static_cast<void*>(&hash));
	
	sph_blake512_init(&ctx_blake);
	sph_blake512(&ctx_blake, static_cast<const void*>(&hash), 64);
	...
	sph_jh512(&ctx_jh, static_cast<const void*>(&hash), 64);
	sph_jh512_close(&ctx_jh, static_cast<void*>(&hash));

	sph_luffa512_init(&ctx_luffa);
	sph_luffa512(&ctx_luffa, static_cast<const void*>(&hash), 64);
	sph_luffa512_close(&ctx_luffa, static_cast<void*>(&hash));
	
	sph_keccak512_init(&ctx_keccak);
	sph_keccak512(&ctx_keccak, static_cast<const void*>(&hash), 64);
	...
	sph_cubehash512(&ctx_cubehash, static_cast<const void*>(&hash), 64);
	...
	sph_hamsi512(&ctx_hamsi, static_cast<const void*>(&hash), 64);
	sph_hamsi512_close(&ctx_hamsi, static_cast<void*>(&hash));

	sph_fugue512_init(&ctx_fugue);
	sph_fugue512(&ctx_fugue, static_cast<const void*>(&hash), 64);
