...
```
git add src/qt/wealthsilostrings.cpp src/qt/locale/wealthsilo_en.ts
git commit
```
...
2. Update `src/qt/wealthsilo_locale.qrc` manually or via
   `ls src/qt/locale/*ts|xargs -n1 basename|sed 's/\(wealthsilo_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'`
3. Update `src/Makefile.qt.include` manually or via
   `ls src/qt/locale/*ts|xargs -n1 basename|sed 's/\(wealthsilo_\(.*\)\).ts/  qt\/locale\/\1.ts \\/'`
...
1. Open `wealthsilo_en.ts` in Qt Linguist (included in the Qt SDK)
...
To create a new language template, you will need to edit the languages manifest file `src/qt/wealthsilo_locale.qrc` and add a new entry. Below is an example of the English language entry.

```xml
<qresource prefix="/translations">
    <file alias="en">locale/wealthsilo_en.qm</file>
    ...
</qresource>
```
