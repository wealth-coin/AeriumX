...
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zAEX mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zAEX Spending Fix
---------------------
The size of zAEX spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zAEX Transaction Recovery
---------------------
Due to the aforementioned issue with zAEX spending, users may find that their attempted spends are now conflicted and zAEX balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
- #292 `feadab4` Additional checks for double spending of zAEX serials. (presstab)
...
- #280 `fdc182d` [Wallet] Fix zAEX spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zAEX balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zAEX control quantity/amount fixes (rejectedpromise)
